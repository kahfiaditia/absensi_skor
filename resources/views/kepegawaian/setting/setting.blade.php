                ...
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                @if ($data_jam->isEmpty())
                                    <a href="{{ route('setting_data.create') }}" type="button"
                                ...
                                <tbody>
                                    @foreach ($data_jam as $as => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $namaHari[$item->id_hari] ?? '-' }}</td>
                                            <td>{{ $item->jam_masuk }}</td>
                                            <td>{{ $item->jam_pulang }}</td>
                                            
                                            <td>
                                                <form class="delete-form"
                                                    action="{{ route('setting_data.destroy', $item->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
